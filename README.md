...
You send a message on Telegram. The ESP32-S3 picks it up over WiFi, feeds it into an agent loop — the LLM thinks, calls tools, reads memory — and sends the reply back. Supports **Anthropic (Claude)**, **OpenAI (GPT)**, and **Longcat** as providers, switchable at runtime. Everything runs on a single $5 chip with all your data stored locally on flash.
...
- An **API key** for your chosen provider:
  ...
  - **Longcat API key** — from [api.longcat.chat](https://api.longcat.chat)
...
#define MIMI_SECRET_MODEL_PROVIDER  "anthropic"     // "anthropic", "openai", or "longcat"
...
mimi> set_api_key sk-ant-api03-... # change API key (Anthropic, OpenAI, or Longcat)
mimi> set_model_provider openai    # switch provider (anthropic|openai|longcat)
