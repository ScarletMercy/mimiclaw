...
static char s_longcat_api_key[LLM_API_KEY_MAX_LEN] = {0};
...
esp_err_t llm_proxy_init(void)
{
    ...
    if (provider_is_longcat() && s_longcat_api_key[0]) {
        ESP_LOGI(TAG, "LLM proxy initialized (provider: longcat, model: %s)", s_model);
    } else if (s_api_key[0]) {
        ESP_LOGI(TAG, "LLM proxy initialized (provider: %s, model: %s)", s_provider, s_model);
    } else {
        ESP_LOGW(TAG, "No API key. Use CLI: set_api_key <KEY> or set_longcat_key <KEY>");
    }
    return ESP_OK;
}
...
esp_err_t llm_set_longcat_key(const char *api_key)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(MIMI_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, MIMI_NVS_KEY_LONGCAT_API_KEY, api_key));
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);
    safe_copy(s_longcat_api_key, sizeof(s_longcat_api_key), api_key);
    ESP_LOGI(TAG, "LongCat API key saved");
    return ESP_OK;
}
