...
/* --- set_longcat_key command --- */
static struct {
    struct arg_str *api_key;
    struct arg_end *end;
} longcat_key_args;

static int cmd_set_longcat_key(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&longcat_key_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, longcat_key_args.end, argv[0]);
        return 1;
    }
    llm_set_longcat_key(longcat_key_args.api_key->sval[0]);
    printf("LongCat API key saved.\n");
    return 0;
}
...
esp_err_t serial_cli_init(void)
{
    ...
    /* set_longcat_key */
    longcat_key_args.api_key = arg_str1(NULL, NULL, "<api_key>", "LongCat API key");
    longcat_key_args.end = arg_end(1);
    esp_console_cmd_t longcat_key_cmd = {
        .command = "set_longcat_key",
        .help = "Set LongCat API key (e.g. set_longcat_key ak_xxx)",
        .func = &cmd_set_longcat_key,
        .argtable = &longcat_key_args,
    };
    esp_console_cmd_register(&longcat_key_cmd);
